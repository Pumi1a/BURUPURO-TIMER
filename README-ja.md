...
## 現時点で把握しているバグ
`/set_channel` の変更がきちんと設定されません。基準時間を決める前に投稿したいチャンネルを指定して下さい。基準時間を決めてしまった方は、`/reset_server` コマンドを実行してから再度指定して下さい。また、`/set_channel` コマンドを実行し、基準時間を即座に投稿されるような時間に設定してみて（現時刻から27 分程前）、設定したチャンネルできちんと投稿されるか確認してみて下さい。、そのうち修正します。
